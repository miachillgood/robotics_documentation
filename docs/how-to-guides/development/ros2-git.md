...
- **Isolated environments**: Each distribution runs in its own Workshop container with
  the correct OS and dependencies
- **Fast switching**: No need to rebuild or reinstall dependencies when
  switching between distributions
- **Clean separation**: Each worktree can have its own Workshop configuration tailored
  to the distribution
